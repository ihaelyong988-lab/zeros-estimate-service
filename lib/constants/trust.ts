...
//                   출처는 30년 현장 실무 누적이다 — 공개 실적 화면이 세는 온라인 접수 DB
//                   실측과 모집단이 다르므로, 라벨로 출처를 갈라 둘을 같은 지표로 읽지 않게 한다.
//
...
export const TRUST_LABEL = {
  confidence: 'AI 신뢰도',
  /** 30년 현장 실무 누적. 출처를 라벨에 적어 온라인 접수 건수와 구분한다. */
  cumulative: '누적 검증(30년 실적)',
  firstReply: '1차 검토 회신',
  saving: '평균 절감 효과',
} as const;

/**
 * 공개 실적 화면(components/PerformanceInsights.tsx) 라벨.
 * 이 화면이 세는 것은 온라인 접수 DB 실측이라 위 누적 검증(30년 실적)과 모집단이 다르다.
 * 같은 이름을 쓰면 방문자가 탭 두 번에 본 서로 다른 두 숫자를 같은 지표로 읽는다.
 */
