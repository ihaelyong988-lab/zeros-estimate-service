...
- **실적 상단 밴드·행간(2026-07-05 최종 — 구 지배지표 조문 대체).** ①좌측 지배 KPI 블록(누적 견적 건수 크게) = **폐지**(사용자 X 마크업·도형 샘플 확정). 최상단 한 행 = **좌 "공종별 견적 실적 분포 (건수·비중)" 헤더 + 우 KPI 4종 균일 클러스터**(견적 건수·검토 비율·평균 소요·공종 수, 라벨 12px·숫자 23px 네이비 — 오렌지·펄스 닷 없음). ②막대 차트 행 pitch = **히트맵 행 pitch와 동기화** — 2026-07-05 압축 확정: 히트맵 셀 `py-1.5`(pitch ≈30px)·막대 `h-[240px]`(구 37px/296px 대체). 목적 = **히트맵 합계 행 첫 화면 노출**이 행간 리듬보다 우선.
