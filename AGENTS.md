...
- **Footer 위치·사업소개 균형·SOP 카피·종료 가드(2026-07-12 확정 — 다항목 지시).** ①FOOTER 소개란(ZEROS 브랜드+핵심영역+©) = **예상견적 의뢰(request) 탭 중앙패널 하단**이 유일 위치(`renderBrandFooter()` 공용 렌더, 위저드 폭 max-w-3xl 정렬·mt-auto 앵커) — **사업소개 탭에서는 삭제됨, 재추가 금지.** ②사업소개는 Footer 여백을 언더라인 이하에 배분: 3단계 타임라인 섹션 = `flex-1 justify-center` + 스텝 gap-9·py-8, 루트 gap-6·py-6, CTA pb-5. ③AI NATIVE 검증 헤드라인 = **"DATA가 도면으로, 검증된 예산안이 되기까지"**(navy + accent span "검증된 예산안" — 컬러 체계 유지, 구 "도면 한 장이…" 폐지) · SOP 화면 최소 폰트 = **12px**(방법론 칩·io 라벨, 11px 금지). ④`TRADE_KEYWORDS`에 `'unknown'` 포함(규모 산정·예산 방향·전문가 상담) — 공사규모·금액 미정 상세도 상단 헤드라인 밴드 표시, **견적규모 4종 모두 키 누락 금지.** ⑤휴대폰 뒤로가기 = AppShell **종료 가드**(진입 엔트리를 `zerosExitGuard`로 치환+동일 URL 한 층 push) → 스택 바닥 popstate 시 "앱을 닫으시겠습니까?" 팝업(닫기/계속 사용, 48px 버튼) — popstate 핸들러에서 가드 state는 applyUrlState 태우지 말 것.
