...
- **실적 KPI 밴드·행간(2026-07-04 확정).** ①KPI 4종(누적 진단 건수 포함) = **동일 폰트**(라벨 12px·숫자 23px), 누적 건수는 **라벨+숫자 한 줄 가로배치**(밑줄 없음, 오렌지는 숫자에만) — 지배지표라고 크게 키우지 않는다. ②막대 분포 차트 행 pitch = **히트맵 행 pitch(≈37px)와 동기화**(8행 = `h-[296px]`, 구 224px 규격 대체) — 상·하 행 리듬을 어긋나게 두지 않는다.
