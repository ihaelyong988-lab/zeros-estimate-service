...
- **§15-3 검증 도구 자체를 의심한다.** 측정값이 조문·상식과 어긋나면 **결함이 아니라 측정 스크립트를 먼저 검사**한다. 선례: 하단 탭 라벨 대비를 3.55:1(미달)로 오판했는데, 원인은 `oklab(...)` 배경을 정규식 숫자 파싱해 거의 검정으로 계산한 것이었다(실제 5.86:1 통과). 대비 측정은 `oklab`/`color()` 표기를 처리하거나 합성 결과색을 쓴다.
  - **2026-08-08 재발 — 같은 함정에 두 번 빠졌다.** ①정규식 파싱으로 3.55 오판(위 선례와 동일) → ②처방이라며 `canvas.fillStyle` 정규화로 바꿨는데 **`fillStyle` 도 `oklab()` 을 못 받아 직전 값(검정)으로 조용히 폴백**해 3.3을 냈다. 잘못된 값이 두 번 다 "그럴듯한 미달 수치"였다. **처방 = 배경 합성을 하지 말고 판정을 뒤집을 수 없는 값으로 잰다** — 글자색 자체(`getComputedStyle().color`)가 목표 색인지 본다. 예: `#9AA3AF`(2.6:1) 잔존 0곳 · `#5B6573`(흰 배경 5.82:1) N곳. 색 문자열 비교는 파싱이 없어 틀릴 여지가 없다. **배경 합성 계산이 필요해지면 그 자체를 먼저 검증하고 쓴다.**
- **§15-4 도구 함정(재발 방지).**
...
- **§15-12 잔여 목록은 단일 문서에 두고, 처리하면 그 턴에 역반영한다 (2026-08-08).** 2026-08-07에 만든 잔여작업 문서가 **하루 만에** 낡았다 — 26건 중 **22건이 이미 해소**돼 있었는데 목록은 그대로였다(코드 실측). 원인 = ①같은 잔여 목록이 문서 3곳(`잔여작업-실행준비.md`·`개선-작업계획서.md §8`·결함대장 부록 B)에 복제돼 있었고 ②처리 기록이 worklog에만 남고 목록에 역반영되지 않았다. 낡은 목록으로 착수하면 **이미 끝난 일을 다시 하고, 없는 파일 줄 수로 소요를 잘못 잡는다**(실제로 `app/page.tsx` 2,166줄 → 1,852줄). 처방 ①**잔여 목록은 `docs/00_orchestration/잔여작업-마스터.md` 하나에만** 둔다(제10조 1정보 1표시) ②잔여를 처리하면 **그 턴에** 마스터의 해당 행을 「해소」로 옮긴다 ③판정 근거는 항상 실측(file:line·grep 건수·테스트·게이트 수치) — "~일 것이다" 금지 ④**착수 전 문서를 믿지 말고 실측을 다시 돌린다**(오늘 22건이 그렇게 나왔다).
- **§15-13 잔여작업은 「누가 손해를 보는가」를 통과해야 한다 (2026-08-08 · 헌법 제5조 재발).** 주인님: *"할 때마다 끊임없이 잔여 작업이 있다고 그러면 뭐가 잘못된 것 아닌가?"* — **지적이 옳았다.** 잔여 6건을 영향으로 다시 재니 C4 10곳 전부 관리자 화면 · N2 관리자 · C6 104곳 중 92곳 관리자 · **D3는 관리자가 2명이어야 발동하는데 실제 1명이라 발생 불가**였다. 고객이 겪는 것은 옅은 글씨 ~10곳(30분)뿐이었다. 근본 원인 = **게이트·린트 경고를 그대로 잔여작업으로 올린 것** — 기계가 트집 잡을 수 있는 것과 사람이 손해 보는 것을 구분하지 않았다. **2026-08-06에 주인님이 같은 지적("MECE 관점")을 했는데 근본을 안 고쳐 재발했다.** 처방 ①목록에 올리기 전 **`누가(고객/주인님/아무도) · 언제(지금/조건부/발생 불가) · 무엇을 잃는가`** 를 한 줄로 적는다. 못 적으면 **올리지 않는다.** ②등급 A(고객이 지금 겪음)=착수 대상 · B(주인님만)=요청 시 · **C(발생 조건 미성립)=목록에서 내린다.** ③**게이트 경고 수치·린트 경고·"조문에 있는데 코드에 없음"·"이론상 경쟁 상태"·"더 나은 구조"는 그 자체로 잔여작업이 아니다.** 기준 문서 = `docs/00_orchestration/잔여작업-마스터.md` §5-A.
