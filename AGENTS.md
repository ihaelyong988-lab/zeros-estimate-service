...
- **실적 KPI 밴드·행간(2026-07-04 확정, 같은 날 재교정 반영).** ①누적 진단 건수 = **지배지표**: 라벨+숫자 **한 줄 가로배치**(밑줄 없음, 오렌지는 숫자에만)하되 **라벨 15px·숫자 30px로 보조보다 크게 — 지배지표를 보조 크기로 축소 금지**(23px 통일안은 사용자 재지적으로 폐기). 보조 3종 = 라벨 12px·숫자 23px 유지. ②막대 분포 차트 행 pitch = **히트맵 행 pitch(≈37px)와 동기화**(8행 = `h-[296px]`, 구 224px 규격 대체) — 상·하 행 리듬을 어긋나게 두지 않는다.
