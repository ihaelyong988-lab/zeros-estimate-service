...
- **유일하게 늘어나는 비용은 문자다.** 본인인증 1건당 1통. 접수 100건이면 100~200통 수준이라 절대액은 작다. 정확한 단가는 solapi 가입 시 확인한다.
- 남용 완화는 코드에 이미 있다 — 번호당 1시간 5회 · IP당 20회(`lib/otp/rateLimit.ts`), 접수 번호당 1시간 3회.
