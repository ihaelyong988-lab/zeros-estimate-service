...
> ## ▶ 현행 상태 (2026-08-01 실측) — 이 작업은 아직 남아 있다
>
...
>
> **`vercel` CLI는 설치돼 있으나 프로젝트 미연결 상태다**(`.vercel/project.json` 없음) → §3의 `vercel login` + `vercel link`가 선행되어야 한다.
>
> **선행 조건은 이미 해소됐다.** 2026-08-01 2단계에서 인증 토큰(`verifiedToken`)이 서버로 전송되는 경로를 만들었으므로, **키를 등록해도 접수가 막히지 않는다.** 그 전에는 등록하는 순간 모든 의뢰 접수가 403으로 실패할 상태였다.
>
> **급하지 않다.** 지금도 접수는 정상 작동한다(인증 게이트를 건너뛰는 상태). 다만 그동안은 **아무나 남의 번호로 접수할 수 있다**는 점을 알고 있어야 한다.
