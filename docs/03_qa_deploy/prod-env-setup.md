...
## 5-A. 비용 (2026-08-08 실측 — 다시 묻지 않도록 기록)

**현재 청구액 0원.** 대시보드·브라우저 창을 열어두는 것 자체는 비용과 무관하다(요청이 있을 때만 과금 대상).

| 항목 | 요금제 | 실측 사용량 | 무료 한도 | 언제 돈이 나가나 |
|---|---|---|---|---|
| Supabase | **Free Plan** | DB 106행 · 파일 **3.12MB**(5개) | DB 500MB · Storage 1GB | 한도 초과 시. 현 속도로는 수년 여유 |
| Vercel | Hobby(무료) 추정 — 대시보드에서 확인 필요 | — | 상업적 사용은 Pro 필요 조건이 있음 | 트래픽·팀 기능 확대 시 |
| 문자(Solapi) | 미가입 | 0건 | 없음(선불) | **가입 후 발송 건당** — 유일한 변동비 |
| 도메인 `zerospipe.co.kr` | — | — | — | 연 1회 갱신 |

### 문자 단가 (2026-08-08 solapi 공식 페이지 2곳 교차 확인 · VAT 미포함)

월 1~9,999건 구간(주인님 해당) 기준. **월 기본료·API 사용료·가입비 없음 — 발송 건당만 청구.**

| 종류 | 단가 | 쓰이는 곳 |
|---|---|---|
| SMS(단문, 45자 이내) | **18원** | 본인인증 번호, 짧은 상태 알림 |
| LMS(장문) | **45원** | 링크·안내가 들어간 상태 알림 |
| 카카오 알림톡 | **13원** | (미도입) 채널 개설+템플릿 심사 필요 |

**접수 1건당 예상**: 본인인증 1통 + 상태 알림 3통(②번 구축 후) ≈ **90~160원**

| 월 접수량 | 월 비용 | 연 비용 |
|---|---|---|
| 10건 | 900~1,600원 | 약 1~2만원 |
| 30건 | 2,700~4,800원 | 약 3~6만원 |
| 100건 | 9,000~16,000원 | 약 11~19만원 |

→ **사업 규모에서 무시할 수준이다.** 볼륨이 커지면 알림톡(13원) 전환으로 더 낮출 수 있다.

- 남용 완화는 코드에 이미 있다 — 번호당 1시간 5회 · IP당 20회(`lib/otp/rateLimit.ts`), 접수 번호당 1시간 3회. **이게 없으면 문자 폭탄이 곧 요금 폭탄이다.**
- ⚠ **실접수는 2건뿐이고 99건이 테스트행(`est-test-*`)이다**(2026-08-08 실측). 실런칭 전 일괄 삭제 대상 — `delete from zeros_estimates where id like 'est-test-%'`.
- ⚠ **Supabase 무료 프로젝트는 일정 기간 요청이 전혀 없으면 일시정지된다.** 라이브 사이트가 살아 있으면 해당되지 않지만, 장기간 트래픽이 끊기면 대시보드에서 수동 재개가 필요하다.
